...
var FiltersService = /** @class */ (function () {
     ...
     */
    function FiltersService(target) {
        /**
    ...
    Object.defineProperty(FiltersService.prototype, "appliedFiltersMap", {
        /**
     ...
     */
    FiltersService.prototype.destroy = function () {
     ...
     */
    FiltersService.prototype.resetValues = function () {
        this.appliedFiltersMap.forEach(function (targetConfig, target) {
            for (var _i = 0, targetConfig_1 = targetConfig; _i < targetConfig_1.length; _i++) {
                var config = targetConfig_1[_i];
                var defaultValue = typeof config.defaultValue === 'function'
                    ? config.defaultValue.call(target)
                    : config.defaultValue;
                var clonedObject = utilities_1.cloneAsLiteral({ defaultValue: defaultValue });
     ...
     */
    FiltersService.prototype.applyParams = function (params) {
        this.appliedFiltersMap.forEach(function (targetConfig, target) {
            for (var _i = 0, targetConfig_2 = targetConfig; _i < targetConfig_2.length; _i++) {
                var config = targetConfig_2[_i];
                if (params && params.hasOwnProperty(config.parameterName) && false === config.ignoreOnAutoMap) {
                    var proposedVal = config.emptyIsNull
                        ? params[config.parameterName] || null
                        : params[config.parameterName];
                    proposedVal = config.coerce ? utilities_1.coerceValue(proposedVal) : proposedVal;
                    target[config.propertyName] = config.parseFormatter
     ...
     */
    FiltersService.prototype.getRequestState = function (filterFn) {
        var result = {};
        this.appliedFiltersMap.forEach(function (targetConfig, target) {
            for (var _i = 0, targetConfig_3 = targetConfig; _i < targetConfig_3.length; _i++) {
                var config = targetConfig_3[_i];
                config = __assign({}, config);
                var proposedVal = target[config.propertyName];
                if (filterFn ? filterFn(config, proposedVal, target) : true) {
                    var resultValue = FiltersService.buildFilterValue(target, proposedVal, config);
     ...
     */
    FiltersService.prototype.registerFilterTarget = function () {
        var _this = this;
        var targets = [];
        ...
        targets.forEach(function (target) {
            _this.appliedFiltersMapInternal.set(target, null);
            if (_this.filtersMapBuilded) {
                _this.buildFilterTargetMap(target);
            }
        });
    };
    /**
     ...
     */
    FiltersService.prototype.removeFilterTarget = function () {
        var _this = this;
        var targets = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            targets[_i] = arguments[_i];
        ...
        targets.forEach(function (target) {
            _this.appliedFiltersMapInternal.delete(target);
        });
    };
    /**
     ...
     */
    FiltersService.prototype.buildFiltersMap = function () {
        var _this = this;
     ...
     *
     */
    FiltersService.prototype.buildFilterTargetMap = function (target) {
        var targetConfig = new Array();
        FiltersService.filterPropertiesMap.forEach(function (typeConfig, type) {
            if (target instanceof type) {
                targetConfig = targetConfig.concat(typeConfig);
                for (var _i = 0, targetConfig_4 = targetConfig; _i < targetConfig_4.length; _i++) {
                    var config = targetConfig_4[_i];
                    if (typeof config.defaultValue === 'undefined') {
                        config.defaultValue = utilities_1.cloneAsLiteral({
     ...
     */
    FiltersService.registerFilterConfig = function (targetType, propertyConfig) {
        var typeConfigs = FiltersService.filterPropertiesMap.has(targetType)
     ...
     */
    FiltersService.buildFilterValue = function (target, value, config) {
        var result = value;
        ...
        result = config && config.coerce ? utilities_1.coerceValue(result) : result;
